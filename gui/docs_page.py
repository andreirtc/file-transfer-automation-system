...
class DocsPageWidget(QWidget):
    ...
    def _doc_safety_locks(self) -> str:
        return """
        <h1>6. Comprehensive System Safety, Validity & Integrity Checks</h1>
        <p>Enterprise server backup operations require absolute reliability. The system incorporates twelve distinct, automated validity and safety checks across every layer of the file lifecycle to prevent partial transfers, corrupt archives, duplicate bandwidth waste, and premature file deletion.</p>

        <h2>1. Multi-Cycle Size Stability Verification</h2>
        <p>Files that are still being written, downloaded, exported, or generated by external applications (e.g. database dumps, accounting reports, video exports) change size over time:</p>
        <ul>
            <li>The system records the file's exact byte count upon discovery.</li>
            <li>It pauses for the configured <b>Stability Check Interval</b> (default: <code>5</code> seconds).</li>
            <li>It re-measures the byte count across <code>2</code> consecutive intervals (configurable via <code>required_stable_checks</code>).</li>
            <li>Only when the byte size remains 100% stable across all consecutive checks is the file declared ready to transfer.</li>
        </ul>

        <h2>2. Modification Timestamp Stability Verification</h2>
        <p>Some software appends data or flushes buffers without changing total file length immediately. The system checks <code>st_mtime</code> across checks. If the modification timestamp continues changing, the file stays in <code>PROCESSING</code> status and is held back from queueing.</p>

        <h2>3. OS-Level Exclusive File Lock Detection (<code>msvcrt.locking</code>)</h2>
        <p>Before any file is touched, the system requests an exclusive low-level Windows file handle using the C runtime non-blocking lock (<code>msvcrt.locking(f.fileno(), msvcrt.LK_NBLCK, 1)</code>):</p>
        <ul>
            <li>If another process (e.g. SQL Server, an archiving utility, or an open editor) holds an open write handle, Windows rejects the lock request.</li>
            <li>The system leaves the file completely untouched and leaves it in <code>DETECTED</code> / <code>PROCESSING</code> status.</li>
            <li>It never interferes with other running server processes.</li>
        </ul>

        <h2>4. Final Pre-Transfer Safety Re-Check</h2>
        <p>Even after a file is declared <code>READY</code>, conditions can change while waiting in line. Milliseconds before the transfer engine reads a single byte, it re-verifies:</p>
        <ol>
            <li>The source file still physically exists on disk.</li>
            <li>Its size matches the exact size measured during stability verification.</li>
            <li>The file can still be opened exclusively without lock contention.</li>
        </ol>
        <p>If any check fails, the transfer safely aborts and the file returns to <code>PROCESSING</code> or <code>FAILED</code> without partial data transmission.</p>

        <h2>5. Real-Time Mid-Compression Deletion Safeguard & Clean Restart</h2>
        <div class="callout-warning">
            <b>Scenario:</b> An operator or automated cron job deletes a file from the source folder while an 87 GB archive is 50% through compression.
        </div>
        <ul>
            <li>A dedicated background watcher polls file existence every 500ms to 1.5s during active compression.</li>
            <li>If a source file vanishes mid-transfer, the compression worker subprocess is terminated immediately.</li>
            <li>The incomplete, partial <code>.zip</code> file is destroyed instantly, preventing corrupted or half-baked archives.</li>
            <li>The deleted file is marked as <code>SKIPPED</code> in the database with the note: <i>"Source file deleted before transfer"</i>.</li>
            <li>The system broadcasts an announcement to the live Activity Feed and restarts compression cleanly with the remaining valid files.</li>
        </ul>

        <h2>6. Incremental Multi-Day File Lifecycle & Job Workspace Tagging</h2>
        <div class="callout-info">
            <b>The "Monday vs. Tuesday" Scenario:</b>
            <br>Monday backup finishes transferring 6 files. On Tuesday, those 6 files are still in the source directory, but 6 new files have been added (12 total). How are they handled and tagged in the Job Workspace?
        </div>
        <ul>
            <li><b>Exact Signature Matching (<code>check_already_transferred</code>):</b> SQLite records each completed file's <code>job_id</code>, exact <code>source_path</code>, exact <code>file_size</code>, and exact <code>source_modified</code> timestamp.</li>
            <li><b>Daily Re-Scan Filtering:</b> When Tuesday's monitoring cycle runs, it compares all 12 files against the database:
                <ul>
                    <li>The <b>6 Monday files</b> match existing <code>COMPLETED</code> records in SQLite. The scanner drops them immediately and will <b>NEVER re-transfer</b> them or bundle them into Tuesday's archive.</li>
                    <li>The <b>6 new files</b> have no database match and are queued with status <code>WAITING_FOR_WINDOW</code> (or <code>READY</code>).</li>
                </ul>
            </li>
            <li><b>Job Workspace Display & Tagging:</b>
                <ul>
                    <li>When you open the <b>Job Workspace</b> tab, the 6 Monday files are clearly visible in the file table tagged with the green <span class="badge" style="background-color: #DCFCE7; color: #15803D;">COMPLETED</span> badge and their Monday completion timestamp.</li>
                    <li>Tuesday's 6 new files appear tagged with <span class="badge" style="background-color: #FFFBEB; color: #92400E;">WAITING_FOR_WINDOW</span> or <span class="badge" style="background-color: #EFF6FF; color: #1D4ED8;">TRANSFERRING</span>, transitioning to <span class="badge" style="background-color: #DCFCE7; color: #15803D;">COMPLETED</span> once Tuesday's batch finishes.</li>
                    <li>If a file was skipped due to duplicate destination detection or operator deletion, it is explicitly tagged with <span class="badge" style="background-color: #F1F5F9; color: #475569;">SKIPPED</span> and explained in the error/remarks column.</li>
                </ul>
            </li>
        </ul>

        <h2>7. Destination Conflict & Collision Detection Policy</h2>
        <p>If a file with the same name already exists at the destination before copying:</p>
        <ul>
            <li>The system compares sizes and cryptographic hashes. If the destination file is identical, the copy is skipped and tagged <code>COMPLETED</code> (or <code>SKIPPED</code>) to avoid redundant network I/O.</li>
            <li>If the file contents differ, the system enforces the configured <b>Overwrite Policy</b>: <code>ask</code> (prompts the operator), <code>overwrite</code> (replaces destination), <code>skip</code> (preserves destination and tags file <code>SKIPPED</code>), or <code>cancel</code>.</li>
        </ul>

        <h2>8. Atomic Hidden Temporary File Staging (Safe Copy)</h2>
        <p>Files and archives are never written directly to their permanent filename. They stream into a hidden temporary file first (<code>.{filename}.transfer_tmp</code>):</p>
        <ul>
            <li>If power fails, network drops, or the process is stopped, only the hidden temporary file is affected.</li>
            <li>The destination folder never contains half-written, unreadable files.</li>
            <li>Only after transfer and integrity checks pass 100% is the temporary file atomically committed using the operating system's <code>os.replace()</code> primitive.</li>
        </ul>

        <h2>9. Dual-Verified Source File Retention & Auto-Cleanup</h2>
        <p>When automated retention cleanup is enabled (<code>auto_cleanup_enabled: true</code>), original source files are purged only after satisfying a strict 4-point verification rule:</p>
        <ol>
            <li>The transfer record is marked <code>COMPLETED</code> in SQLite.</li>
            <li>The completion timestamp is older than the configured <code>auto_cleanup_days</code> (1 to 365 days).</li>
            <li>The destination file physically exists on the target storage server.</li>
            <li>The destination file size matches the source file size down to the exact byte.</li>
        </ol>

        <h2>10. Cryptographic End-to-End SHA-256 Integrity Verification</h2>
        <p>Transfers are never assumed correct based on file size alone. The system independently computes full 256-bit SHA-256 cryptographic hashes for both source and destination copies using dynamic 8 MB memory buffers. If even a single bit differs, the temporary destination file is erased and the job is marked <code>FAILED</code>.</p>

        <h2>11. 64-Bit Unlimited Zip64 Architecture (100+ GB Scaling)</h2>
        <p>Upgraded to Qt native <code>qint64</code> signals and Python <code>force_zip64=True</code> streaming:</p>
        <ul>
            <li>Eliminates the legacy 32-bit signed integer limit (2.14 GB) that previously caused <code>OverflowError</code> on massive files.</li>
            <li>Bypasses the standard 4 GB ZIP member ceiling, supporting 87 GB, 100+ GB, and multi-terabyte files up to <b>16 Exabytes</b>.</li>
            <li>Bundles <code>pyzipper</code> directly in the executable for native AES-256 encryption with Zip64 extensions.</li>
        </ul>

        <h2>12. Microsoft Excel Workbook File Lock Safeguard (<code>[Errno 13]</code>)</h2>
        <p>When generating the daily checklist report (<code>reports/TFSPH_Daily_Backup_Checklist_YYYY-MM-DD.xlsx</code>), if an operator or auditor has the workbook currently open in Microsoft Excel, Windows places an exclusive lock on the file:</p>
        <ul>
            <li>The system catches Windows <code>[Errno 13] Permission denied</code> without crashing.</li>
            <li>It automatically writes the report to a fallback file: <code>..._latest.xlsx</code>.</li>
            <li>It alerts the operator with an in-app Warning banner. Once Excel is closed, generating the report updates the master workbook cleanly.</li>
        </ul>
        """
